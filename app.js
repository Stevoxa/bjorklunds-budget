...
/**
 * Kant-svep i huvudflik: linjär ordning (ingen cirkulär wrap).
 * Ordning: Analys → Intäkter → Utgifter → Spara → Inställningar.
 * Vänsterkant V→H: ett steg mot Analys (stopp på Analys).
 * Högerkant H→V: ett steg mot Inställningar (stopp på Inställningar), bara utan öppen undersida/modal.
 */
const APP_MAIN_NAV_ORDER = ["analysis", "incomes", "expenses", "savings", "settings"];
...
/**
 * Vänsterkant → svep högerut: undersidor (steg tillbaka / Avbryt), därefter ett steg mot Analys i huvudmenyn (linjärt, stopp på Analys).
 */
function appEdgeSwipeTryConsumeFromLeftEdge() {
  ...
  const idx = APP_MAIN_NAV_ORDER.indexOf(route);
  if (idx <= 0) return false;
  const prev = APP_MAIN_NAV_ORDER[idx - 1];
  const prevHash = `#/${prev}`;
...
/**
 * Högerkant → svep vänsterut: ett steg mot Inställningar (linjärt, stopp på Inställningar), endast utan öppen undersida/modal.
 */
function appEdgeSwipeTryConsumeFromRightEdge() {
  ...
  const route = ui.activeRoute || parseRouteFromHash().route;
  const idx = APP_MAIN_NAV_ORDER.indexOf(route);
  if (idx < 0 || idx >= APP_MAIN_NAV_ORDER.length - 1) return false;
  const next = APP_MAIN_NAV_ORDER[idx + 1];
  const nextHash = `#/${next}`;
...
function initAppEdgeSwipeNavigation() {
  ...
  let startT = 0;
  let active = false;
  /** "left" = vänsterkant V→H (mot Analys); "right" = högerkant H→V (mot Inställningar). */
  let edgeSide = null;
  let lastConsumeAt = 0;
...
async function initVaultBootstrap() {
  ...
  const vaultRestartConfirmBtn = document.getElementById("vaultRestartConfirmBtn");
  ...
  let vaultLockSplashRevoke = "";
  const splashImg = document.getElementById("vaultLockSplashImg");
  if (splashImg instanceof HTMLImageElement) {
    ...
    if (vaultLockSplashRevoke) {
      try {
        URL.revokeObjectURL(vaultLockSplashRevoke);
      } catch {
        /* ignore */
      }
      vaultLockSplashRevoke = "";
    }
    if (blob && blob.size > 0) {
      vaultLockSplashRevoke = URL.createObjectURL(blob);
      splashImg.src = vaultLockSplashRevoke;
    } else {
  ...
  return new Promise((resolve) => {
    ...
    const showSetupChoice = (fromUnlockRestart) => {
      showErr("");
    ...
    vaultBackFromCreate.onclick = () => showSetupChoice(enteredSetupFromUnlock);
    vaultBackFromImport.onclick = () => showSetupChoice(enteredSetupFromUnlock);
...
async function syncSettingsVaultBiometricUi() {
  const V = globalThis.BjorkVault;
...
function wireVaultBiometricSettingsOnce() {
  if (vaultBiometricSettingsWired) return;
  vaultBiometricSettingsWired = true;
  const en = document.getElementById("settingsVaultBiometricEnableBtn");
  const dis = document.getElementById("settingsVaultBiometricDisableBtn");
  if (en) {
...
function renderSettingsPage() {
  ...
  void syncSettingsVaultBiometricUi();
}

let settingsAutosaveWired = false;
let vaultBiometricSettingsWired = false;
...
function initActions() {
  ...
  async function runBackupImportWithPassphrase() {
    ...
    if (!pass) {
      renderErrorSummary(errEl, [{ label: "Ange lösenordet för backup-filen.", jumpId: "backupImportSheetPassphrase" }]);
      return;
    }
    const V = globalThis.BjorkVault;
    if (!V || !env) {
    ...
    if (!r.ok) {
      if (r.error === "schema") {
        ...
        renderErrorSummary(errEl, [
          {
            label: "Fel lösenord eller skadad fil. Kontrollera lösenordet och försök igen.",
